    ...
    if (
      ...
      if (err.apiError.message === 'Repository creation failed.') {
        if (
          ...
          throw new Error(
            `您在 ${account.friendlyEndpoint} 的账号上已经有一个名为 "${name}" 的仓库了。\n\n` +
              '请删除那个仓库再试一次。'
          )
        }
      }
...
async function pushRepo(
  ...
  const pushTitle = `将仓库推送到 ${account.friendlyEndpoint}`
...
export async function createTutorialRepository(
  ...
  progressCb(`在 ${account.friendlyEndpoint} 上创建仓库`, 0)
  ...
  await git(['commit', '-m', 'Initial commit'], path, 'tutorial:commit') // Desktop-CN: 提交名称请勿翻译
