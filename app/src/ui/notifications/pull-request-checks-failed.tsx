...
> {
  ...
  public render() {
    ...
    const pluralChecks = failedChecks.length > 1 ? '个检查' : '个检查'
  ...
  private renderSummary() {
    const failedChecks = this.state.checks.filter(isFailure)
    const pluralThem = failedChecks.length > 1 ? '' : ''
    return (
      <div className="footer-question">
        <span>是否切换到此拉取请求并开始修复{pluralThem}？</span>
      </div>
    )
  }
  ...
  private renderCheckRunStepsLoading(): JSX.Element {
    return (
      <div className="loading-check-runs">
        ...
        <div className="call-to-action">检查步骤正在加载中</div>
      </div>
    )
  }
