...
> {
  ...
  private renderOkButtonText() {
    ...
    if (isCreateBranchState) {
      return __DARWIN__ ? '摘取到新分支' : '摘取到新分支'
    }

    const pluralize = this.props.commitCount > 1 ? '个提交' : '个提交'
    const okButtonText = `摘取${this.props.commitCount}${pluralize}`
